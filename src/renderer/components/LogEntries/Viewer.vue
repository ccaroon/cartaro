<template>
  <v-dialog :value="value" persistent scrollable max-width="75%">
    <v-card>
      ...
      <v-card-text v-html="$markdown.render(logEntry.content || '')" class="pt-3" style="height: 500px"></v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
        <v-chip small label class="pa-1 mr-1" v-for="(tag, idx) in logEntry.tags" :key="idx">{{ tag }}</v-chip>
        <v-spacer></v-spacer>
      </v-card-actions>
    </v-card>
  </v-dialog>
