<template>
  <v-dialog :value="value" persistent scrollable max-width="75%">
    <v-card>
      ...
      <v-card-text v-html="$markdown.render(secret.note || '')" class="pt-3" style="height: 250px"></v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
        <v-chip small label class="pa-1 mr-1" v-for="(tag, idx) in secret.tags" :key="idx">{{ tag }}</v-chip>
        <v-spacer></v-spacer>
        <v-btn fab small>
          <v-icon :color="hideData ? 'red' : 'green'" @click="hideData = !hideData">mdi-{{ hideData ? "lock" :
          "lock-open" }}</v-icon>
        </v-btn>
      </v-card-actions>
    </v-card>
