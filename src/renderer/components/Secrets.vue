<template>
  <v-container>
    <AppBar v-bind:name="'Secrets'" v-bind:numPages="Math.ceil(totalSecrets / perPage)" v-bind:refresh="refresh"
      v-bind:buttons="appBarButtons"></AppBar>
    <SecretEditor v-model="showEditor" v-bind:secret="secret" v-on:close="closeEditor"></SecretEditor>
    <SecretViewer v-model="showViewer" v-bind:secret="secret" v-bind:isHidden="isHidden[secret.id]"
      v-on:close="closeViewer"></SecretViewer>
    <v-list dense>
      <v-list-group v-for="(secretList, name, idx) in secrets" :key="name" :class="utils.rowColor(idx)"
        :prepend-icon="icons.superSearch(name, 'mdi-lock').code">
        <template v-slot:activator>
          <v-list-item-title>{{ name }}</v-list-item-title>
        </template>
        <v-list-item v-for="(secret, idx) in secretList" :key="secret.id" :class="utils.rowColor(idx)" @click.stop>
          <v-list-item-avatar>
            <v-icon :color="isHidden[secret.id] ? 'red' : 'green'"
              @click="$set(isHidden, secret.id, !isHidden[secret.id])">mdi-{{ isHidden[secret.id] ? "lock" : "lock-open"
              }}</v-icon>
          </v-list-item-avatar>
          <v-list-item-content @click="view(secret)">
            <v-list-item-title :class="secret.isDeleted() ? 'text-decoration-line-through' : ''">
              {{ secret.name }} |
              <span class="text-caption text--secondary">{{ secret.system }} / {{ secret.sub_system }}</span>
            </v-list-item-title>
            <v-list-item-subtitle>
              <v-row dense no-gutters>
                ...
                <v-col>
                  <Tags v-bind:tags="secret.tags" v-bind:color="utils.rowColor(idx + 1)"></Tags>
                </v-col>
              </v-row>
            </v-list-item-subtitle>
          </v-list-item-content>
